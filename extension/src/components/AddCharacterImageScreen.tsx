...
import type { PendingCharacterImageAdd } from '../types'
...
export function AddCharacterImageScreen({ capture, onSaved, onBack }: Props) {
  ...
  useEffect(() => {
    supabase
      .from('characters')
      .select('id, name, cover_image_url, tags')
    ...
    <div className="flex flex-col flex-1 overflow-hidden min-h-0">
      ...
      <div className="shrink-0 px-3 py-2 border-b border-zinc-700">
        <input
          ...
          autoFocus
          className="w-full px-2.5 py-1.5 rounded-md bg-zinc-900 border border-zinc-700 text-zinc-100 placeholder-zinc-600 text-sm focus:outline-none focus:border-violet-500 transition-colors"
        />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1.5 space-y-1">
        {loading ? (
          ...
          <p className="text-xs text-zinc-500 text-center py-6">
            {search ? 'Keine Treffer' : 'Noch keine Charaktere vorhanden'}
          </p>
        ) : (
          filtered.map(char => (
            <button
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {(char.tags ?? []).length > 0 && (
                  <p className="text-[10px] text-zinc-500 truncate">{char.tags.slice(0, 3).join(' · ')}</p>
                )}
              </div>
              {selectedId === char.id && <span className="text-violet-400 text-sm shrink-0">✓</span>}
            </button>
          ))
        )}
      </div>
      ...
      <div className="px-2 pb-2 pt-1 border-t border-zinc-700 shrink-0">
        ...
        >
          {saved ? '✓ Hinzugefügt!' : sicherung ? sicherung : saving ? 'Speichern…' : selectedId ? 'Bild hinzufügen' : 'Charakter auswählen'}
        </button>
      </div>
    </div>
  )
