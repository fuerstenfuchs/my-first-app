...
const CATEGORIES = [
  ...
  { key: 'eventlocation', label: 'Event',         emoji: '🎭' },
  { key: 'nachtlocation', label: 'Nacht',         emoji: '🌃' },
...
export function LocationCaptureScreen({ capture, onSaved, onBack }: Props) {
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      const result = await analyzeAsset(capture.imageUrl, 'location', session?.access_token ?? null, appUrl, croppedDataUrl ?? undefined)
    ...
    <div className="flex flex-col flex-1 overflow-hidden min-h-0">
            ...
            <div className="relative">
              ...
              <div className="absolute bottom-1.5 right-1.5 flex gap-1">
                ...
                <button
                  ...
                  disabled={analyzing || saving}
                  className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-teal-600/90 hover:bg-teal-500 disabled:opacity-50 text-white text-[10px] font-medium transition-colors"
                >
                  {analyzing
      ...
      {(capture.sourceUrl || capture.sourceTitle) && (
        <div className="shrink-0 px-3 py-1.5 border-b border-zinc-700/60 bg-zinc-900/40 flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-teal-400 truncate">{sourceDomain}</span>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-2.5 space-y-2.5">
        ...
        <div>
          ...
          <input
            ...
            placeholder="z.B. Reeperbahn Hamburg"
            className="w-full px-2 py-1.5 rounded-md bg-zinc-900 border border-zinc-700 text-zinc-100 placeholder-zinc-600 text-sm focus:outline-none focus:border-teal-500 transition-colors"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Atmosphäre, Lichtstimmung…"
            rows={2}
            className="w-full px-2 py-1.5 rounded-md bg-zinc-900 border border-zinc-700 text-zinc-100 placeholder-zinc-600 text-sm focus:outline-none focus:border-teal-500 transition-colors resize-none"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="nacht, regen, neon, urban"
            className="w-full px-2 py-1.5 rounded-md bg-zinc-900 border border-zinc-700 text-zinc-100 placeholder-zinc-600 text-sm focus:outline-none focus:border-teal-500 transition-colors"
          />
        </div>
