...
export function PoseCaptureScreen({ capture, onSaved, onBack }: Props) {
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      const result = await analyzeAsset(capture.imageUrl, 'pose', session?.access_token ?? null, appUrl, croppedDataUrl ?? undefined)
  ...
  async function handleSave() {
    ...
    if (hasImage && capture.imageUrl) {
      ...
      const r = await bildSichern(capture.imageUrl, 'pose')
      ergebnisse.push(r)
      coverUrl = r.url
    }
    ...
    if (croppedDataUrl) {
      ...
      const r = await bildSichern(croppedDataUrl, 'pose')
      ergebnisse.push(r)
      cropUrl = r.url
    }
    setSicherung(null)
    ...
    <div className="flex flex-col flex-1 overflow-hidden min-h-0">
      ...
      <div className="px-2 pb-2 pt-1 border-t border-zinc-700 shrink-0">
        <button onClick={handleSave} disabled={saving || saved || !name.trim()}
          ...
          {saved ? '✓ Gespeichert!' : sicherung ? sicherung : saving ? 'Speichern…' : '🎭 Als Pose / Aktion speichern'}
        </button>
      </div>
    </div>
  )
