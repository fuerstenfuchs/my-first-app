...
export function useLocationDetail(locationId: string | null) {
  ...
  async function deleteImage(variantId: string, imageId: string, _storagePath: string | null): Promise<void> {
    ...
    const { data: weg, error } = await supabase
      .from('location_images').delete().eq('id', imageId).select('url').maybeSingle()
    ...
    if (url && locationId) {
      ...
      const titel = titelbildNachLoeschen('locations', location?.cover_image_url, url, rest)
      if (titel.aendern) {
        const { error: titelErr } = await supabase
          .from('locations').update({ cover_image_url: titel.neu }).eq('id', locationId)
        if (titelErr) toast.error('Titelbild konnte nicht nachgezogen werden')
        else setLocation(prev => prev ? { ...prev, cover_image_url: titel.neu } : prev)
      }
    }
