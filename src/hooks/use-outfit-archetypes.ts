...
export interface OutfitArchetypeAttributes {
  kategorie?:    string
  farben?:       string
  material?:     string
  muster?:       string
...
export interface OutfitArchetypeImage {
  id:           string
  archetype_id: string
  user_id:      string
  url:          string
...
export interface OutfitArchetypeInput {
  name:               string
...
const BUCKET = 'outfit-archetype-images'
const TABLE  = 'outfit_archetypes'
const IMAGE_TABLE = 'outfit_archetype_images'

function normalize(raw: Record<string, unknown>): OutfitArchetype {
  return {
    id:                raw.id as string,
    ...
    attributes:        (raw.attributes as OutfitArchetypeAttributes) ?? {},
...
function normalizeImage(raw: Record<string, unknown>): OutfitArchetypeImage {
  return {
    id:           raw.id as string,
...
export function useOutfitArchetypes() {
  const [archetypes, setArchetypes] = useState<OutfitArchetype[]>([])
  ...
  const fetch = useCallback(async () => {
    ...
    if (error) {
      toast.error('Fehler beim Laden der Outfit Archetypes')
    } else {
  ...
  async function createArchetype(input: OutfitArchetypeInput): Promise<OutfitArchetype | null> {
    const supabase = createClient()
  ...
  async function updateArchetype(id: string, input: OutfitArchetypeInput): Promise<boolean> {
    const supabase = createClient()
    const { data, error } = await supabase
      .from(TABLE)
      .update({
...
// ─── useOutfitArchetypeImages ───────────────────────────────────────────────

export function useOutfitArchetypeImages(archetypeId: string | null, onCoverSynced?: (id: string, url: string | null) => void) {
  const [images, setImages] = useState<OutfitArchetypeImage[]>([])
  ...
  async function syncCoverIfFirst(nextImages: OutfitArchetypeImage[]) {
