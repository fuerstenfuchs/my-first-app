...
import {
  ...
  type PoseAction, type PoseActionVariant,
...
function PoseActionCard({
  poseAction, categoryEmoji, isSelected, onClick, onEdit, onDelete,
}: {
  ...
  return (
    ...
    >
        ...
        {poseAction.source_url && (
          <div className="absolute bottom-1.5 left-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
            <span className="text-[9px] bg-black/60 text-white/80 px-1 py-0.5 rounded">
              {(() => { try { return new URL(poseAction.source_url).hostname.replace('www.', '') } catch { return '' } })()}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="px-2 py-2">
        <p className="text-xs font-medium leading-tight truncate">{poseAction.name}</p>
        {poseAction.tags.length > 0 && (
          <p className="text-[10px] text-muted-foreground/70 truncate mt-0.5">
            {poseAction.tags.slice(0, 3).join(' · ')}
          </p>
        )}
      </div>
    </button>
...
export default function PoseActionsPage() {
  const { poseActions, loading, createPoseAction, updatePoseAction, deletePoseAction, patchPoseActionCover } = usePoseActions()

  const [selectedCategory, setSelectedCategory] = useState<PoseCategory>('stehend')
  const [selectedPoseActionId, setSelectedPoseActionId] = useState<string | null>(null)
  const [search, setSearch] = useState('')

  const [formOpen, setFormOpen]               = useState(false)
  const [editingPoseAction, setEditingPoseAction] = useState<PoseAction | null>(null)
  const [deletePoseActionId, setDeletePoseActionId] = useState<string | null>(null)

  const {
    poseAction, variants, loading: detailLoading, uploading,
    createVariant, updateVariant, deleteVariant, reorderVariants,
    uploadImages, addImageUrl, deleteImage, reorderImages, updatePoseActionCover,
    refetch: refetchDetail,
  } = usePoseActionDetail(selectedPoseActionId)

  const [variantFormOpen, setVariantFormOpen]     = useState(false)
  const [editingVariant, setEditingVariant]       = useState<PoseActionVariant | null>(null)
  const [deleteVariantId, setDeleteVariantId]     = useState<string | null>(null)
  const [selectedVariantId, setSelectedVariantId] = useState<string | null>(null)

  const [aiAnalyzing, setAiAnalyzing] = useState(false)
  const [aiSuggestion, setAiSuggestion] = useState<{
    name: string; category: PoseCategory; tags: string[]; description: string
  } | null>(null)
  ...
  const categoryCounts = useMemo(() => {
    ...
    for (const pa of poseActions) map[pa.category] = (map[pa.category] ?? 0) + 1
    return map
  }, [poseActions])
  ...
  async function handlePoseActionSave(input: PoseActionInput, coverFile?: File | null) {
    if (editingPoseAction) {
      return updatePoseAction(editingPoseAction.id, input)
    }
    const pa = await createPoseAction(input, coverFile)
    if (pa) { setSelectedCategory(pa.category); setSelectedPoseActionId(pa.id) }
    return pa
  }

  async function handleVariantSave(input: PoseActionVariantInput, files: File[]) {
  ...
  async function handleAnalyzePose() {
    if (!poseAction?.cover_image_url) return
    setAiAnalyzing(true)
    ...
    try {
      ...
      setAiSuggestion({
        ...
        category:    (validCategories.includes(result.category as PoseCategory) ? result.category : poseAction.category) as PoseCategory,
        tags:        result.tags        ?? poseAction.tags,
        description: result.description ?? poseAction.description ?? '',
      })
    } catch (err) {
  ...
  async function handleApplySuggestion() {
    if (!poseAction || !aiSuggestion) return
    const ok = await updatePoseAction(poseAction.id, {
  ...
  const currentCategory = POSE_CATEGORIES.find(c => c.key === selectedCategory)!
    ...
    <div className="flex h-svh min-w-0 overflow-hidden">
        ...
        <nav className="flex-1 overflow-y-auto p-2 space-y-0.5">
          {POSE_CATEGORIES.map(cat => {
            ...
            return (
              <button
                key={cat.key}
                onClick={() => { setSelectedCategory(cat.key); setSelectedPoseActionId(null); setSearch('') }}
                className={cn(
              ...
              >
                ...
                {count > 0 && (
                  <span className={cn('text-[11px] tabular-nums shrink-0', isActive ? 'text-purple-400' : 'text-muted-foreground/40')}>
                    {count}
                  </span>
                )}
              </button>
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        <header className="border-b shrink-0 px-4 py-2.5 flex items-center gap-3">
          ...
          <Button size="sm" className="shrink-0 ml-auto" onClick={() => { setEditingPoseAction(null); setFormOpen(true) }}>
            <Plus className="mr-1.5 h-3.5 w-3.5" />Neue Pose
          </Button>
        </header>
        ...
        <div className="flex-1 overflow-hidden relative">
          <div className="absolute inset-y-0 left-0 overflow-y-auto overflow-x-hidden p-4" style={{ right: '-17px' }}>
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center min-h-[320px] gap-4 text-center">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-medium text-muted-foreground">
                    {search ? 'Keine Pose gefunden' : `Noch keine ${currentCategory.label}-Posen`}
                  </p>
                  {!search && (
                    <p className="text-xs text-muted-foreground/60">
                      Rechtsklick auf ein Bild im Browser → „Als Pose/Aktion speichern"
                    </p>
                  )}
                </div>
                {!search && (
                  <Button size="sm" variant="outline" onClick={() => { setEditingPoseAction(null); setFormOpen(true) }}>
              ...
              <div className={cn('grid gap-3', detailOpen ? 'grid-cols-2 lg:grid-cols-3' : 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5')}>
                {filteredPoseActions.map(pa => (
                  <PoseActionCard
                    key={pa.id}
                    poseAction={pa}
                    categoryEmoji={currentCategory.emoji}
                    isSelected={selectedPoseActionId === pa.id}
                    onClick={() => { setSelectedPoseActionId(prev => prev === pa.id ? null : pa.id); setSelectedVariantId(null) }}
                    onEdit={() => { setEditingPoseAction(pa); setFormOpen(true) }}
                    onDelete={() => setDeletePoseActionId(pa.id)}
                  />
                ))}
              </div>
            )}
              ...
              <div className="flex-1 overflow-hidden relative">
                <div className="absolute inset-y-0 left-0 overflow-y-auto overflow-x-hidden" style={{ right: '-17px' }}>
                    ...
                    {poseAction.cover_image_url ? (
                      <>
                        ...
                        >
                          ...
                          {aiAnalyzing ? 'Analysiere…' : 'Pose analysieren'}
                        </button>
                        {poseAction.source_url && (
                      ...
                      <div className="space-y-1.5 text-xs">
                        ...
                        <div className="flex gap-2">
                          ...
                          <span className="text-foreground/90">
                            {POSE_CATEGORIES.find(c => c.key === aiSuggestion.category)?.emoji}{' '}
                            {POSE_CATEGORIES.find(c => c.key === aiSuggestion.category)?.label}
                          </span>
                        </div>
                  ...
                  <div className="px-4 py-3 border-b border-border space-y-2">
                    ...
                    {poseAction.description && (
                      <p className="text-xs text-muted-foreground leading-relaxed">{poseAction.description}</p>
                    )}
                    {poseAction.source_url && (
                      <a href={poseAction.source_url} target="_blank" rel="noopener noreferrer"
                    ...
                    {poseAction.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {poseAction.tags.map(tag => <Badge key={tag} variant="secondary" className="text-[10px] px-1.5 py-0">{tag}</Badge>)}
                      </div>
                    )}
                  </div>
                  ...
                  {selectedVariant ? (
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      {selectedVariant.images.length > 0 ? (() => {
                        ...
                        const isCover = poseAction.cover_image_url === currentImg.url
                        return (
                          <>
                            <div className="relative bg-black/20 rounded-xl overflow-hidden group/img">
                              ...
                              {isCover ? (
                                ...
                                <button
                                  onClick={() => updatePoseActionCover(currentImg.url, newUrl => { if (selectedPoseActionId) patchPoseActionCover(selectedPoseActionId, newUrl) })}
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      {variants.length === 0 ? (
                        <p className="text-xs text-muted-foreground/50 text-center py-4">
                          z.B. Langsam, Schnell, Elegant — als separate Varianten anlegen.
                        </p>
                      ) : (
      ...
      <PoseActionForm
        open={formOpen}
        onClose={() => { setFormOpen(false); setEditingPoseAction(null) }}
        poseAction={editingPoseAction}
        defaultCategory={selectedCategory}
        onSave={handlePoseActionSave}
      />
      <PoseActionVariantForm
        open={variantFormOpen}
        onClose={() => { setVariantFormOpen(false); setEditingVariant(null) }}
        variant={editingVariant}
        defaultName={editingVariant ? undefined : poseAction?.name}
        onSave={handleVariantSave}
      />

      <AlertDialog open={!!deletePoseActionId} onOpenChange={open => !open && setDeletePoseActionId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Pose / Aktion löschen?</AlertDialogTitle>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
              onClick={async () => {
                if (!deletePoseActionId) return
                await deletePoseAction(deletePoseActionId)
                if (selectedPoseActionId === deletePoseActionId) setSelectedPoseActionId(null)
                setDeletePoseActionId(null)
