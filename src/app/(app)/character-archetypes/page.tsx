...
import { CharacterArchetypeForm } from '@/components/character-archetypes/character-archetype-form'
import { CharacterArchetypeSheetDialog } from '@/components/character-archetypes/character-archetype-sheet-dialog'
import {
  useCharacterArchetypes, useCharacterArchetypeImages,
  type CharacterArchetype, type CharacterArchetypeInput, type CharacterArchetypeAttributes,
} from '@/hooks/use-character-archetypes'
import { cn } from '@/lib/utils'

const EMOJI = '👤'

const ATTRIBUTE_LABELS: Record<keyof CharacterArchetypeAttributes, string> = {
  geschlecht: 'Geschlecht', alter: 'Alter', koerperbau: 'Körperbau', groesse: 'Größe',
  haarfarbe: 'Haarfarbe', haarstil: 'Haarstil', augenfarbe: 'Augenfarbe', bart: 'Bart',
  hauttyp: 'Hauttyp', nationalitaet: 'Nationalität', beruf: 'Beruf',
  persoenlichkeit: 'Persönlichkeit', ausstrahlung: 'Ausstrahlung', stimmung: 'Stimmung',
...
function ArchetypeCard({
  ...
  item: CharacterArchetype; isSelected: boolean
...
function ImageGrid({ archetypeId, onCoverSynced }: { archetypeId: string; onCoverSynced: (id: string, url: string | null) => void }) {
  const { images, loading, uploading, uploadImages, deleteImage } = useCharacterArchetypeImages(archetypeId, onCoverSynced)
...
export default function CharacterArchetypesPage() {
  const { archetypes, loading, createArchetype, updateArchetype, deleteArchetype, patchCover } = useCharacterArchetypes()

  const [search, setSearch] = useState('')
  const [formOpen, setFormOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<CharacterArchetype | null>(null)
  ...
  async function handleSave(input: CharacterArchetypeInput) {
    if (editingItem) {
  ...
  const attributeEntries = selectedItem
    ? (Object.entries(selectedItem.attributes) as [keyof CharacterArchetypeAttributes, string][]).filter(([, v]) => v?.trim())
    : []
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        <header className="border-b shrink-0 px-4 py-2.5 flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="font-semibold text-sm">{EMOJI} Character Archetypes</span>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center min-h-[320px] gap-4 text-center">
                ...
                <p className="text-sm font-medium text-muted-foreground">
                  {search ? 'Kein Archetyp gefunden' : 'Noch kein Character Archetype'}
                </p>
                <p className="text-xs text-muted-foreground/60 max-w-xs">
                  Lege universelle Charakter-Typen wie „Schlagerstar", „Polizistin" oder „Cowboy" an —
