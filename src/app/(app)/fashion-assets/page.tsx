...
function GalleryCard({
  asset, categoryEmoji, isSelected, onClick, onEdit, onDelete,
}: {
  asset: FashionAsset
    ...
    >
      ...
      <div className="aspect-[3/4] bg-muted/30 relative overflow-hidden">
        {asset.cover_image_url ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-1 text-muted-foreground/25">
        ...
        {asset.source_url && (
          <div className="absolute bottom-1.5 left-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
            <span className="text-[9px] bg-black/60 text-white/80 px-1 py-0.5 rounded">
              {(() => { try { return new URL(asset.source_url).hostname.replace('www.', '') } catch { return '' } })()}
            </span>
          </div>
        )}
      </div>
...
export default function FashionAssetsPage() {
  ...
  async function handleAssetSave(input: FashionAssetInput, coverFile?: File | null) {
  ...
  async function handleVariantSave(input: FashionAssetVariantInput, files: File[]) {
    ...
    <div className="flex h-svh min-w-0 overflow-hidden">
      ...
      <div className="w-48 shrink-0 flex flex-col border-r border-border">
        <header className="border-b shrink-0 px-3 py-3 flex items-center gap-2">
          <SidebarTrigger />
          <span className="text-sm font-semibold flex-1 truncate">Fashion</span>
          <Button
          ...
          {FASHION_CATEGORIES.map(cat => {
            ...
            return (
              ...
              >
                ...
                {count > 0 && (
                  <span className={cn('text-[11px] tabular-nums shrink-0', isActive ? 'text-rose-400' : 'text-muted-foreground/40')}>
                    {count}
                  </span>
                )}
              </button>
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        <header className="border-b shrink-0 px-4 py-2.5 flex items-center gap-3">
          ...
          <Button size="sm" className="shrink-0 ml-auto" onClick={() => { setEditingAsset(null); setAssetFormOpen(true) }}>
                ...
                <div className="space-y-1">
                  ...
                  {!search && (
                    <p className="text-xs text-muted-foreground/60">
                      Rechtsklick auf ein Bild im Browser → „Als Fashion Asset speichern"
                    </p>
                  )}
                </div>
                {!search && (
              ...
              <div className={cn('grid gap-3', detailOpen ? 'grid-cols-2 lg:grid-cols-3' : 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5')}>
                {filteredAssets.map(a => (
                  <GalleryCard
                    ...
                    onClick={() => { setSelectedAssetId(prev => prev === a.id ? null : a.id); setSelectedVariantId(null) }}
                    onEdit={() => { setEditingAsset(a); setAssetFormOpen(true) }}
          ...
          {detailLoading ? (
            ...
            <>
              ...
              <div className="border-b shrink-0 px-3 py-2.5 flex items-center gap-1.5">
                ...
                <Button size="icon" variant="ghost" className="h-7 w-7 shrink-0 text-destructive hover:text-destructive" onClick={() => setDeleteAssetId(asset.id)}>
                  <Trash2 className="h-3.5 w-3.5" />
                </Button>
                <Button size="icon" variant="ghost" className="h-7 w-7 shrink-0" onClick={() => { setSelectedAssetId(null); setSelectedVariantId(null) }}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
                  ...
                  <div className="px-4 py-3 border-b border-border space-y-2">
                    ...
                    {asset.source_url && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-rose-400 transition-colors"
                      >
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      {variants.length === 0 ? (
                        <p className="text-xs text-muted-foreground/50 text-center py-4">
                          Keine Varianten — das Cover-Bild ist das Hauptbild.
                        </p>
                      ) : (
