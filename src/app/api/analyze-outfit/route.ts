  ...
  try {
    ...
    if (body.imageBase64) {
      ...
      if (!mime.startsWith('image/')) {
        return NextResponse.json({ error: 'Die URL verweist auf kein gültiges Bild.' }, { status: 422, headers: CORS_HEADERS })
      }
    ...
    const message = await client.messages.create({
      ...
      messages: [{
        ...
        content: [
          ...
          { type: 'text', text: 'Analyze this outfit and return the JSON.' },
        ],
      }],
    })
    ...
    console.error('analyze-outfit error:', msg)
