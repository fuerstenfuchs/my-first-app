...
import { OUTFIT_ATTRIBUTE_OPTIONS } from '@/lib/archetype-attribute-options'
import type { OutfitArchetype, OutfitArchetypeInput, OutfitArchetypeAttributes } from '@/hooks/use-outfit-archetypes'

interface Props {
  open:    boolean
  onClose: () => void
  item?:   OutfitArchetype | null
  onSave:  (input: OutfitArchetypeInput) => Promise<unknown>
}

const ATTRIBUTE_FIELDS: { key: keyof OutfitArchetypeAttributes; label: string }[] = [
  ...
  { key: 'formalitaet',  label: 'Formalität' },
  { key: 'land',         label: 'Land' },
  { key: 'region',       label: 'Region' },
  { key: 'epoche',       label: 'Epoche' },
]

export function OutfitArchetypeForm({ open, onClose, item, onSave }: Props) {
  const isEdit = !!item
  ...
  const [attributes, setAttributes]              = useState<OutfitArchetypeAttributes>({})
  ...
  function setAttr(key: keyof OutfitArchetypeAttributes, value: string) {
  ...
  async function handleGenerate() {
    ...
    setAiError(null)
    try {
      const res = await fetch('/api/generate-outfit-archetype', {
        method: 'POST',
      ...
      const result = await res.json() as {
        ...
        attributes?: OutfitArchetypeAttributes
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!name.trim()) return
    setSaving(true)
    const input: OutfitArchetypeInput = {
      name:               name.trim(),
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEdit ? 'Archetyp bearbeiten' : 'Neuer Outfit Archetype'}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-1">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder="z.B. Schlager Outfit, Polizei Uniform, Business Look"
                required
                className="flex-1"
              />
          ...
          <div className="space-y-2.5">
            ...
            {ATTRIBUTE_FIELDS.map(f => (
              <AttributeField
                ...
                options={OUTFIT_ATTRIBUTE_OPTIONS[f.key].options}
                allowCustom={OUTFIT_ATTRIBUTE_OPTIONS[f.key].allowCustom}
                multi={OUTFIT_ATTRIBUTE_OPTIONS[f.key].multi}
              />
            ))}
          </div>
          ...
          {/* Short description */}
          <div className="space-y-1.5">
            <Label htmlFor="oa-short">Kurze Beschreibung</Label>
            <Textarea
              id="oa-short"
              value={shortDescription}
              onChange={e => setShortDescription(e.target.value)}
              placeholder="z.B. Glamouröses Schlager-Bühnenoutfit mit roten Pailletten."
              rows={2}
          ...
          {/* Long description */}
          <div className="space-y-1.5">
            <Label htmlFor="oa-long">Lange Beschreibung</Label>
            <Textarea
              id="oa-long"
              value={longDescription}
              onChange={e => setLongDescription(e.target.value)}
              placeholder="Ausführliche Beschreibung von Schnitt, Material und Styling…"
              rows={3}
              className="resize-none"
            />
          </div>
          ...
          {/* Prompt */}
          <div className="space-y-1.5">
            <Label htmlFor="oa-prompt">Prompt</Label>
            <Textarea
              id="oa-prompt"
              value={prompt}
          ...
          {/* Tags */}
          <div className="space-y-1.5">
            <Label htmlFor="oa-tags">Tags</Label>
            <Input
              id="oa-tags"
              value={tagsRaw}
              onChange={e => setTagsRaw(e.target.value)}
              placeholder="schlager, bühne, glamour, pailletten"
            />
          </div>
