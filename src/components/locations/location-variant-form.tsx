...
interface Props {
  open: boolean
  onClose: () => void
  variant?: LocationVariant | null
  defaultName?: string
  onSave: (input: LocationVariantInput, files: File[]) => Promise<boolean | LocationVariant | null>
}

export function LocationVariantForm({ open, onClose, variant, defaultName, onSave }: Props) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="z.B. Tag, Nacht, Regen, Winter, Sommer…"
              required
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="lv-desc">Beschreibung</Label>
            <Input
              id="lv-desc"
              value={description}
